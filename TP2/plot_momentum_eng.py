...
bars1 = ax.bar(x_pos - width/2, original_errors, width, label='Without Momentum', color='#1f77b4', edgecolor='black', linewidth=0.5)
bars2 = ax.bar(x_pos + width/2, momentum_errors, width, label='With Momentum ($\\alpha$=0.9)', color='#ff7f0e', edgecolor='black', linewidth=0.5)
...
ax.set_ylabel('Final Error (after 2M cycles)', fontsize=11)
ax.set_title('8-bit Parity Check: Final Error Comparison With/Without Momentum', fontsize=13)
ax.set_xticks(x_pos)
ax.set_xticklabels([f'{n}' for n in hidden_sizes])
ax.legend(fontsize=10)
ax.grid(True, axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('8bit_momentum_comparison_eng.png', dpi=150, bbox_inches='tight')
plt.close()
...
ax.plot(cycles_orig, errors_orig, linewidth=1.5, label='Without Momentum (final: {:.2f})'.format(errors_orig[-1]), color='#1f77b4')
ax.plot(cycles_mom, errors_mom, linewidth=1.5, label='With Momentum $\\alpha$=0.9 (final: {:.2f})'.format(errors_mom[-1]), color='#ff7f0e')
ax.set_title('8-bit Parity Check (8 Hidden Neurons): Effect of Momentum', fontsize=13)
...
plt.tight_layout()
plt.savefig('8bit_momentum_j8_curve_eng.png', dpi=150, bbox_inches='tight')
plt.close()
...
ax.set_title('8-bit Parity Check (With Momentum): Extended Hidden Neuron Experiment', fontsize=13)
ax.set_xlabel('Number of Hidden Neurons', fontsize=11)
ax.set_ylabel('Final Error (after 2M cycles)', fontsize=11)
ax.grid(True, axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('8bit_momentum_extended_eng.png', dpi=150, bbox_inches='tight')
plt.close()

print("English momentum comparison graphs generated.")
