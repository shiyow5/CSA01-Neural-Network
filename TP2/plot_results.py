...
fig.suptitle('4-bit パリティチェック問題：隠れニューロン数による学習曲線の比較', fontsize=14)
...
for idx, J in enumerate(hidden_sizes):
    ...
    if fc:
        ...
        ax.annotate(f'収束: {fc}サイクル', xy=(fc, fe), fontsize=8,
                    xytext=(0.55, 0.5), textcoords='axes fraction',
                    arrowprops=dict(arrowstyle='->', color='gray'),
                    bbox=dict(boxstyle='round,pad=0.3', fc='lightyellow'))
    ax.set_title(f'隠れニューロン数 J = {J}', fontsize=11)
    ax.set_xlabel('学習サイクル数')
    ax.set_ylabel('誤差 (Error)')
    ax.set_yscale('log')
    ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('4bit_learning_curves.png', dpi=150, bbox_inches='tight')
plt.close()

# --- 4-bit: 全隠れ層を一枚にまとめた比較図 ---
fig, ax = plt.subplots(figsize=(10, 6))
for idx, J in enumerate(hidden_sizes):
    cycles, errors, fc, fe = parse_log(f'j_{J}.txt')
    ax.plot(cycles, errors, color=colors[idx], linewidth=1.5, label=f'J = {J} (収束: {fc}サイクル)' if fc else f'J = {J}')
ax.set_title('4-bit パリティチェック：隠れニューロン数による学習曲線の比較', fontsize=13)
...
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('4bit_comparison.png', dpi=150, bbox_inches='tight')
plt.close()

# --- 8-bit parity check ---
fig, axes = plt.subplots(2, 2, figsize=(12, 9))
fig.suptitle('8-bit パリティチェック問題：隠れニューロン数による学習曲線の比較', fontsize=14)
...
for idx, J in enumerate(hidden_sizes):
    ...
    # 最終誤差を注釈
    if errors:
        ax.annotate(f'最終誤差: {errors[-1]:.3f}', xy=(0.5, 0.95),
                    xycoords='axes fraction', fontsize=9, ha='center',
                    bbox=dict(boxstyle='round,pad=0.3', fc='lightyellow'))

plt.tight_layout()
plt.savefig('8bit_learning_curves.png', dpi=150, bbox_inches='tight')
plt.close()

# --- 8-bit: 全隠れ層を一枚にまとめた比較図 ---
fig, ax = plt.subplots(figsize=(10, 6))
for idx, J in enumerate(hidden_sizes):
    cycles, errors, fc, fe = parse_log(f'j8_{J}.txt')
    final_err = errors[-1] if errors else 0
    ax.plot(cycles, errors, color=colors[idx], linewidth=1.5,
            label=f'J = {J} (最終誤差: {final_err:.2f})')
ax.set_title('8-bit パリティチェック：隠れニューロン数による学習曲線の比較', fontsize=13)
ax.set_xlabel('学習サイクル数', fontsize=11)
ax.set_ylabel('誤差 (Error)', fontsize=11)
ax.set_yscale('log')
ax.legend(fontsize=10)
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('8bit_comparison.png', dpi=150, bbox_inches='tight')
plt.close()
...
ax.set_title('4-bit パリティチェック：収束に要した学習サイクル数', fontsize=13)
ax.set_xlabel('隠れニューロン数', fontsize=11)
ax.set_ylabel('収束サイクル数', fontsize=11)
ax.grid(True, axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('4bit_convergence_bar.png', dpi=150, bbox_inches='tight')
plt.close()
...
ax.set_title('8-bit パリティチェック：200万サイクル後の最終誤差', fontsize=13)
ax.set_xlabel('隠れニューロン数', fontsize=11)
ax.set_ylabel('最終誤差', fontsize=11)
ax.grid(True, axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('8bit_final_error_bar.png', dpi=150, bbox_inches='tight')
plt.close()

print("全てのグラフを生成しました:")
print("  - 4bit_learning_curves.png")
print("  - 4bit_comparison.png")
print("  - 4bit_convergence_bar.png")
print("  - 8bit_learning_curves.png")
print("  - 8bit_comparison.png")
print("  - 8bit_final_error_bar.png")
